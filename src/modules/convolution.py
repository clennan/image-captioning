...
def fprop_next_conv(x, f, w, b, batch_size):
    x_dims = x.get_shape().as_list()
    deconv_shape = tf.stack([batch_size]+x_dims[1:])

    v = tf.nn.relu(w)
    z = tf.nn.conv2d(x, v, strides=[1, 1, 1, 1], padding='SAME') + tf.maximum(b, 0)
    s = tf.divide(f, z)
    c = tf.nn.conv2d_transpose(s, v, deconv_shape, strides=[1, 1, 1, 1], padding='SAME')
